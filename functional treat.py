...
def display_summary():
    ...
    Calculates and displays total elements, max, min, sum, and average.
    """
    if data:
        print("\nData Summary:")
...
def filter_data():
    """
    Filter dataset values based on a threshold using lambda and filter().
    """
    if data:
...
def dataset_statistics(data):
    """
    Calculate basic statistical values for a dataset and return a tuple.
    """
    minimum = min(data)
    maximum = max(data)
    total = sum(data)
